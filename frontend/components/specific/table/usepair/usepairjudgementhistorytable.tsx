...
// services
import { deleteUsepair, useFetchHistory, useFetchPagedHistoryUsePairJudgements } from "../../../../lib/service/judgement/JudgementResource";
...
const UsePairJudgementHistoryTable: React.FC<{ phase: Phase }> = ({ phase }) => {

    const storage = useStorage();
    const [page, setPage] = useState(0);
    const usepairjudgements = useFetchPagedHistoryUsePairJudgements(phase?.getId().getOwner(), phase?.getId().getProject(), phase?.getId().getPhase(), page, !!phase);
...
export default UsePairJudgementHistoryTable;
