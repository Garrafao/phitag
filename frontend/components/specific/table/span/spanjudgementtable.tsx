...
import { deleteLexSub, deleteSpan, useFetchPagedLexSubJudgements, useFetchPagedSpanJudgements } from "../../../../lib/service/judgement/JudgementResource";
...
import SpanJudgement from "../../../../lib/model/judgement/spanjudgement/model/SpanJudgement";
import DeleteSpanJudgementCommand from "../../../../lib/model/judgement/spanjudgement/command/DeleteSpanJudgementCommand";

const SpanJudgementTable: React.FC<{ phase: Phase, modalState: { open: boolean, callback: Function } }> = ({ phase, modalState }) => {
    ...
    const judgements = useFetchPagedSpanJudgements(phase?.getId().getOwner(), phase?.getId().getProject(), phase?.getId().getPhase(), page, !!phase);

    const [editModal, setEditModal] = useState({
        open: false,
        judgement: null as unknown as SpanJudgement,
    });

    const deleteCallback = (judgement: SpanJudgement) => {
        deleteSpan(
            new DeleteSpanJudgementCommand(
    ...
    useEffect(() => {
        if (judgements.isError && router.isReady) {
    ...
    }, [judgements.isError, router.isReady]);
    ...
    return (
        <div>
            ...
            <AddJudgementToPhaseModal isOpen={modalState.open} closeModalCallback={modalState.callback} phase={phase} mutateCallback={judgements.mutate} />
{/*             <EditLexSubJudgementModal isOpen={editModal.open} closeModalCallback={() => setEditModal({ open: false, judgement: null as unknown as SpanJudgement })} judgement={editModal.judgement} mutateCallback={judgements.mutate} />
 */}        </div>
    );
}

export default SpanJudgementTable;
