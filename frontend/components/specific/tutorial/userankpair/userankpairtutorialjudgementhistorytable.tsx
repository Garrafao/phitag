...
import { useFetchPagedHistoryUsePairTutorialJudgements, useFetchPagedHistoryUseRankPairTutorialJudgements, useFetchPagedHistoryUseRankTutorialJudgements } from "../../../../lib/service/tutorial/tutorialresources";
...
const UseRankPairTutorialJudgementHistoryTable: React.FC<{ phase: Phase }> = ({ phase }) => {

    const storage = useStorage();
    const [page, setPage] = useState(0);
    const judgements = useFetchPagedHistoryUseRankPairTutorialJudgements(phase?.getId().getOwner(), phase?.getId().getProject(), phase?.getId().getPhase(), page, !!phase);
                ...
                <table className="min-w-full border-b-[1px] border-base16-gray-200 divide-y divide-base16-gray-200">
                    ...
                    <tbody className=" text-base16-gray-700">
                        {judgements.data.getContent().map((judgement, i) => {
                            let userankjudgement: UseRankPairJudgement = judgement;
                            return (
                               <> 
                                <tr key={userankjudgement.getId().getId()}>
                                 ...
                                 {expandedInstances[i] && (
                                    <tr key={`${i}-expanded`}
                                        ...
                                        <td colSpan={6}>
                                            <UsageCardPair isOpen={true} userankpairinstance={userankjudgement.getInstance()} />
                                        </td>
                                    </tr>
                                )}
                            </>
...
export default UseRankPairTutorialJudgementHistoryTable;
