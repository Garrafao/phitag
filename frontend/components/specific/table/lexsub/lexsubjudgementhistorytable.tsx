...
const LexSubJudgementHistoryTable: React.FC<{ phase: Phase, modalState: { open: boolean, callback: Function } }> = ({ phase, modalState }) => {
    ...
    const lexsubjudgements = useFetchPagedHistoryLexSubJudgements(phase?.getId().getOwner(), phase?.getId().getProject(), phase?.getId().getPhase(), page, !!phase);
...
export default LexSubJudgementHistoryTable;
