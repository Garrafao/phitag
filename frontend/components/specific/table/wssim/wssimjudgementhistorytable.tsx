...
const WSSIMJudgementHistoryTable: React.FC<{ phase: Phase }> = ({ phase }) => {

    const storage = useStorage();
    const [page, setPage] = useState(0);
    const wssimjudgements = useFetchPagedHistoryWSSIMJudgements(phase?.getId().getOwner(), phase?.getId().getProject(), phase?.getId().getPhase(), page, !!phase);
...
export default WSSIMJudgementHistoryTable;
