...
import {  useFetchPagedSpanInstance } from "../../../../lib/service/instance/InstanceResource";
...
const SpanInstanceTable: React.FC<{ phase: Phase, modalState: { openData: boolean, callbackData: Function, openGenerate: boolean, callbackGenerate: Function } }> = ({ phase, modalState }) => {

    const [page, setPage] = useState(0);
    const spaninstances = useFetchPagedSpanInstance(phase?.getId().getOwner(), phase?.getId().getProject(), phase?.getId().getPhase(), page, !!phase);


    // Reload the data on reload
    useEffect(() => {
        spaninstances.mutate();
    }, [phase]);

    if (!phase || spaninstances.isLoading || spaninstances.isError) {
                ...
                <div className="overflow-auto">
                    <table className="min-w-full border-b-[1px] border-base16-gray-200 divide-y divide-base16-gray-200">
                        ...
                        <tbody className=" text-base16-gray-700">
                            {spaninstances.data.getContent().map((instance, i) => {
                                return (
 ...
 */}
            ...
            <PageChange page={page} maxPage={spaninstances.data.getTotalPages()} pageChangeCallback={(p: number) => { setPage(p) }} />

            <AddInstanceToPhaseModal isOpen={modalState.openData} closeModalCallback={modalState.callbackData} phase={phase} mutateCallback={spaninstances.mutate} />
            <GenerateInstancesForPhaseModal isOpen={modalState.openGenerate} closeModalCallback={modalState.callbackGenerate} phase={phase} mutateCallback={spaninstances.mutate} additional={false} additionalFileName="" />
    
        </div>
    );
}

export default SpanInstanceTable;
